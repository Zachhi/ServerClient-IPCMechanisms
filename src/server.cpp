...
void process_newchannel_request (RequestChannel *_channel){
	...
	RequestChannel *dataChannel = nullptr;
	//open the correct channel type. default to fifo if input is f or anythhing other than s or q
	if(chanType == "s")
		dataChannel = new SHMRequestChannel(new_channel_name, SHMRequestChannel::SERVER_SIDE, buffercapacity);
	else if (chanType == "q")
		dataChannel = new MQRequestChannel(new_channel_name, MQRequestChannel::SERVER_SIDE, buffercapacity);
	else
		dataChannel = new FIFORequestChannel(new_channel_name, FIFORequestChannel::SERVER_SIDE);

	thread thread_for_client (handle_process_loop, dataChannel);
...
int main(int argc, char *argv[]){
	...
	if(chanType == "s")
		...
		control_channel= new MQRequestChannel("control", MQRequestChannel::SERVER_SIDE, buffercapacity);
	else
